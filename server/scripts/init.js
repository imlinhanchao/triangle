...
async function main () {
  ...
  config.mail['host'] = await rl.inputData('Email Server Host', config.mail['host']);
  config.mail['port'] = parseInt(await rl.inputData('Email Server Port', config.mail['port']));
  config.mail.auth['account'] = await rl.inputData('Mail Account', config.mail.auth['account']);
  config.mail.auth['pass'] = await rl.inputData('Acount Password', config.mail.auth['pass']);
